...
```{r, echo=FALSE, fig.height = 6, fig.width = 8, fig.cap = '**Figure 2.** (a) Absolute gape height (mm) and (b) relative gape height, calculated as gape height (mm) / standard length (mm) for all species ordered within each functional group by decreasing absolute gape height. A single outlier (64 mm) for *Cephalopholis argus* is not shown.'}
...
all_fg_GH_graph_df <- 
  all_fg_GH_graph_df %>% 
    ...
    dplyr::mutate(lme_slope = test_gh_summ$estimate) %>% 
...
```{r, echo=FALSE, fig.height=8, fig.width=7, fig.cap='**Figure 5.** Gape height ~ standard length relationships for the (a) six zooplanktivores and (b) six energy$-$sharing herbivorous and detritivorous species, in order of increasing slope. Plot details are the same as in Figure 4.'}
# With "Figure" label
master_layout <- 
...
master_layout <- 
grid.layout(nrow = 4, ncol = 4, 
            widths = unit(c(0.1, 1.2, 1, 1), "null"), 
            heights = unit(c(1, 0.15, 1, 0.15), "null"))

grid.newpage()
pushViewport(viewport(layout = master_layout))
print(pisc_prey, vp = set_vp(1, 2))
print(pisc_prey_lut, vp = set_vp(1, 3))
print(pisc_prey_ser, vp = set_vp(1, 4))
print(benth_prey, vp = set_vp(3, 2))
print(benth_prey_mul, vp = set_vp(3, 3))
grid.text(
  expression( paste("Standard length (", mm, ")", sep = "") ), 
  #vp = viewport(layout.pos.row = 2, layout.pos.col = 2:4), 
...
grid.text(
  "Prey total length (mm)",
  vp = viewport(layout.pos.row = 1, layout.pos.col = 1), 
  gp = gpar(fontsize = 10), rot = 90, vjust = 1.6
  )
...
pushViewport(viewport(layout = master_layout))
print(pisc_prey, vp = set_vp(1, 2))
print(pisc_prey, vp = set_vp(1, 3:4))
print(benth_prey, vp = set_vp(3, 2))
print(benth_prey, vp = set_vp(3, 3))
grid.text(
  expression( paste("Standard length (", mm, ")", sep = "") ), 
  #vp = viewport(layout.pos.row = 2, layout.pos.col = 2:4), 
...
grid.newpage()
pushViewport(viewport(layout = master_layout))
print(pisc_prey_fg, vp = set_vp(1, 2))
print(benth_prey_fg, vp = set_vp(3, 2))
grid.text(
  expression( paste("Standard length (", mm, ")", sep = "") ), 
  #vp = viewport(layout.pos.row = 2, layout.pos.col = 2:4), 
