...
groupwise_lm <- function(df, variable) {
  lm <- dlply(df, .(variable), function(z) {
    lm(log(gh)~log(SLMM), data=z)
  })
}

test <- groupwise_lm(fish, fish$SpeciesCode)
ldply(test, coef)

groupwise_lm_gh_SL <- function(df, variable, y, x) {
  lm <- with(data=df, dlply(df, .(df$variable), function(z) {
    lm(log(gh)~log(SLMM), data=z))
  }))
}

groupwise_lm <- function(df, variable, y, x) {
  lm <- do.call( "ddply", list(df, variable, function(z) {
    lm(log(y)~log(x), data=z)
  }))
}
...
count_spp <- function(df) {
  do.call(ddply(.data = df, .(SpeciesCode), summarize, 
        n     = paste("n ==", length(SpeciesCode)))
}

count_spp <- function(df, variable) {
  do.call( "ddply", list(df, variable, summarize, n = paste("n ==", call("length", variable))))
}

z <- do.call("ddply",list(df, SpeciesCode, summarize, Ave = call("mean",as.symbol(NewColName),na.rm=TRUE)))
return(z)
}
...
write_lme_groups <- function(summ_df, variable) {
  ...
  len <- length(variable)
  for (i in (1:len)) {
      l <- list(a = format(df[i, 3], digits=2),
                b = format(df[i, 2], digits=2), 
                r2 = format(df[i, 4], digits=2)
      )
      if (l$b >= 0) {
        eqn <- substitute(log~italic(y) ==
                            a%.% log~italic(x) + b*~~italic(r)^2~"="~r2, l)
      }
      
      else {
        l <- list(a = format(df[i, 3], digits=2),
                  b = format(abs(df[i, 2]), digits=2), 
                  r2 = format(df[i, 4], digits=2)
        )
        eqn <- substitute(log~italic(y) ==
                            a%.% log~italic(x) - b*~~italic(r)^2~"="~r2, l)
      ...
      lm_eq <- as.character(as.expression(eqn))
      print(c(as.character(df[i,1]), lm_eq))
...
dfSample <- (write_lme_groups(test, test$variable))
...
lm_gh_df <- groupwise_lm_gh(df=p, variable=p$SpeciesCode)
summ_df  <- write_lme_groups(lm_df=lm_gh_df, variable=lm_gh_df$variable)
colnames(summ_df) <- c("SpeciesCode", "eqn")
...
spp1 <-
  ...
  facet_wrap(~SpeciesCode, ncol=4, drop=F) +
  geom_smooth(method=lm) +
  geom_text(data=summ_df, aes(x=4.7, y=4.7, label=eqn), parse=T) +
  ggtitle("VerticalGape")
facet_grid(. ~ cyl, labeller = label_both)
...
lm_gh_df <- groupwise_lm_gh(p, variable=p$SpeciesCode)
summ_df <- write_lme_groups(lm_gh_df, variable=lm_gh_df$variable)
summ_df

lm_df <- lm_gh_df


lm <- lm(log(gh)~log(SLMM), p)
confint(lm)

do.call("as.character", list(lm_df$int, lm_df$slope, lm_df$rsq))

write_lme_groups <- function(lm_df, variable) {
  df <- as.data.frame(do.call(cbind, lm_df))
  df <- lm_df
  ...
  for (i in (1:len)) {
    ...
    #m[i,] <- c(as.character(df[i,1], lm_eq))
    m <- rbind(m, c(as.character(df[i,1]), lm_eq))
  }
}

test2 <- as.data.frame(do.call(cbind, lm_gh_df))
test2
