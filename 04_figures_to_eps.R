...
P_prey_SL <- mutate(P_prey_SL, j_fg = 'All Piscivores')
...
df.n <- ddply(.data=filter(P_prey_SL, Family == 'Lutjanidae'), .(j_fg), summarize, n=paste("n ==", length(SL)))
pisc_prey_lut <- 
ggplot(data = filter(P_prey_SL, Family == 'Lutjanidae'), aes(x = SL, y = pSize)) +
  geom_point(aes(shape = pType)) +
...
df.n <- ddply(.data=filter(P_prey_SL, Family == 'Serranidae'), .(j_fg), summarize, n=paste("n ==", length(SL)))
pisc_prey_ser <- 
ggplot(data = filter(P_prey_SL, Family == 'Serranidae'), aes(x = SL, y = pSize)) +
  geom_point(aes(shape = pType)) +
  geom_point(aes(x = 510, y = 100), alpha = 0.0) +
  geom_point(aes(x = 49, y = 258), alpha = 0.0) +
  stat_quantile(geom = "quantile", quantiles = c(0.10, 0.50, 0.90), method = "rq", 
...
B_prey_SL <- mutate(B_prey_SL, j_fg = 'All Benthic Invertivores')
...
benth_prey_mul <- 
ggplot(data = filter(B_prey_SL, Family == 'Mullidae'), aes(x = SL, y = pSize)) +
  geom_point(aes(shape = pType)) +
  geom_point(aes(x = 510, y = 100), alpha = 0.0) +
  geom_point(aes(x = 49, y = 258), alpha = 0.0) +
  scale_shape_manual(values=c(1, 19)) +
...
#fig.height=3.2, fig.width=11, fig.cap='**Figure 1.** Quantile regressions (10th, 50th, and 90th quantiles) of the relationships between prey total length (mm) and predator standard length (mm) for (a) all piscivores (68 predator individuals) and the two most sampled piscivore families (b) Lutjanidae and (c) Serranidae and (d) all benthic invertivores (77 predator individuals) and the most sampled benthic invertivore (e) *Parupeneus insularis* (68 predator individuals). Each point represents a single fish (open circles) or invertebrate (solid circles) prey item found in a predator stomach. The sample size of prey items plotted are labelled on the plot as n = number of prey items. Prey size is the measured length (mm) of the intact or partially digested prey items, and hence taken to be the minimum prey size.'}

# Piscivores: n = 92 prey items from 68 individuals
# Benthic invertivores: n = 108 prey items from 77 individuals
# Lutjanidae: (n = 30 prey items from 20 individuals)
# Serranidae: (n = 54 prey items from 44 individuals)
# Parupeneus insularis: (n = 99 prey items from 68 individuals)

dev.new(height = 3.2, width = 11)
master_layout <- 
grid.layout(nrow = 2, ncol = 10, 
...
grid.text(
  ...
  gp = gpar(fontsize = 10), vjust = -10.5 #-13
  )
grid.text(
...
grid.text(
  "c)", vp = viewport(layout.pos.row = 1, layout.pos.col = 5), 
  gp = gpar(fontsize = 10), vjust = -10.5 #-13
  )
grid.text(
  "d)", vp = viewport(layout.pos.row = 1, layout.pos.col = 7), 
  gp = gpar(fontsize = 10), vjust = -10.5 #-13
  )
grid.text(
  "e)", vp = viewport(layout.pos.row = 1, layout.pos.col = 9), 
  gp = gpar(fontsize = 10), vjust = -10.5 #-13
  )
#grid.text(
#  "Figure 1", vp = viewport(layout.pos.row = 2, layout.pos.col = 1), 
#  gp = gpar(fontsize = 10), vjust = 1, hjust = -0.5
#  )

dev.copy2eps(file = 'Figures/Figure_1.eps')
...
#fig.height = 6, fig.width = 8, fig.cap = '**Figure 2.** (a) Absolute gape height (mm) and (b) relative gape height, calculated as gape height (mm) / standard length (mm) for all species ordered within each functional group by decreasing absolute gape height. A single outlier (64 mm) for *Cephalopholis argus* is not shown.'}
...
grid.text("a)", vp = viewport(layout.pos.row = 1, layout.pos.col = 1), 
    gp = gpar(fontsize = 10), hjust = 0, vjust = -10)
grid.text("b)", vp = viewport(layout.pos.row = 3, layout.pos.col = 1), 
    gp = gpar(fontsize = 10), hjust = 0, vjust = -16)

# Figure label
#grid.text("Figure 2", vp = viewport(layout.pos.row = 4, layout.pos.col = 1), 
#    gp = gpar(fontsize = 10), hjust = 0, vjust = 0)

dev.copy2eps(file = 'Figures/Figure_2.eps')
...
all_spp_GH <- sma(gh ~ SL * SpeciesCode, data = new_pento, log = "xy", 
...
#gh_lm <- lme4::lmer(slope ~ fg + 0 + (1 | Family), data = all_spp_summ_df_gh)
gh_lm <- lm(slope ~ fg + 0, data = all_spp_summ_df_gh)
gh_lm_ci <- confint(gh_lm)

gh_lm_summ <- data_frame(fg = factor(c('Pi', 'BI', 'ZP', 'HE/DE'), levels = c('Pi', 'BI', 'ZP', 'HE/DE')), 
                        estimate = summary(gh_lm)$coefficients[, 1],
                        lwr = gh_lm_ci[, 1], 
                        upr = gh_lm_ci[, 2])
...
gh_lm_summ$start <- c(0.0, 1.2, 1.8, 3.0)
gh_lm_summ$end   <- c(1.0, 1.6, 2.8, 4.0)

gh_plot <- 
ggplot(all_spp_summ_df_gh2) +
  geom_rect(data = gh_lm_summ, aes(xmin = start - 0.05, xmax = end + 0.05, ymin = lwr, ymax = upr), fill = "gray85") +
  ...
  geom_segment(data=gh_lm_summ, aes(y = estimate, yend = estimate, x = start - 0.05, xend = end + 0.05), colour = '#099DFFFF', size = 1, lineend = 'round') +
...
#gw_lm <- lme4::lmer(slope ~ fg + 0 + (1 | Family), data = all_spp_summ_df_gw)
gw_lm <- lm(slope ~ fg + 0, data = all_spp_summ_df_gw)
gw_lm_ci <- confint(gw_lm)

gw_lm_summ <- data_frame(fg = factor(c('Pi', 'BI', 'ZP', 'HE/DE'), 
  levels = c('Pi', 'BI', 'ZP', 'HE/DE')), 
                        estimate = summary(gw_lm)$coefficients[, 1],
                        lwr = gw_lm_ci[, 1], 
                        upr = gw_lm_ci[, 2])
...
gw_lm_summ$start <- c(0.0, 1.2, 1.8, 3.0)
gw_lm_summ$end   <- c(1.0, 1.6, 2.8, 4.0)

gw_plot <- 
ggplot(all_spp_summ_df_gw2) +
  geom_rect(data = gw_lm_summ, aes(xmin = start - 0.05, xmax = end + 0.05, ymin = lwr, ymax = upr), fill = "gray85") +
  #geom_point(aes(x = -0.2, y = 1), alpha = 0) +
  geom_point(aes(x = spacing, y = slope), size = 1, colour = "gray50") + 
  theme_bw() +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
  ...
  theme(axis.text.x = element_text(angle=45, colour="black", hjust=1, size = 8), 
        ...
        axis.title.y = element_blank(), 
        axis.ticks.length = unit(-0.1, "cm")) +
  geom_segment(data=gw_lm_summ, aes(y = estimate, yend = estimate, x = start - 0.05, xend = end + 0.05), colour = '#099DFFFF', size = 1, lineend = 'round') +
  geom_hline(aes(yintercept = 1), colour = 'red', linetype = 2) +
  #scale_x_continuous(breaks = 0.5) +
...
#fig.height=7, fig.width=5.2, fig.cap='**Figure 3.** Comparison of allometric coefficients across functional groups, calculated using a linear model for (a) gape height ~ standard length and (b) gape width ~ standard length relationships. Estimates of functional group mean slopes are shown by the blue line, species mean slopes are plotted as grey points, and confidence limits are bounded by the grey box. For reference, isometry (slope = 1) is plotted as the red dashed line.'}

# CODE FOR VISREG STYLE FG COMPARISON PLOT

dev.new(height = 7, width = 5.2)
master_layout <- 
...
grid.text(
  "a)", vp = viewport(layout.pos.row = 1, layout.pos.col = 1), 
  gp = gpar(fontsize = 10), vjust = -12
  )
grid.text(
  "b)", vp = viewport(layout.pos.row = 2, layout.pos.col = 1), 
  gp = gpar(fontsize = 10), vjust = -16
  )
grid.text(
  "Gape height slope estimate",
  vp = viewport(layout.pos.row = 1, layout.pos.col = 1),
  rot = 90, gp = gpar(fontsize = 10), vjust = 1, hjust = 0.6
    )
grid.text(
  "Gape width slope estimate",
  vp = viewport(layout.pos.row = 2, layout.pos.col = 1),
  rot = 90, gp = gpar(fontsize = 10), vjust = 1, hjust = 0.2
    )
grid.text(
    "Piscivore",
    vp = viewport(layout.pos.row = 1, layout.pos.col = 3),
    vjust = -12, gp = gpar(fontsize = 10), hjust = 0.1
    )
grid.text(
    expression("  Benthic \nInvertivore"),
    vp = viewport(layout.pos.row = 1, layout.pos.col = 4),
    vjust = -11, gp = gpar(fontsize = 10), hjust = 0.22
    )
grid.text(
    "Zooplanktivore",
    vp = viewport(layout.pos.row = 1, layout.pos.col = 5),
    vjust = -12, gp = gpar(fontsize = 10), hjust = 0.45
    )
grid.text(
    "Herbivore/\nDetritivore",
    vp = viewport(layout.pos.row = 1, layout.pos.col = 6),
    vjust = -3.75, gp = gpar(fontsize = 10), hjust = 0.55
    )
#grid.text("Functional group", vp = viewport(layout.pos.row = 4, layout.pos.col = 3:6),
    #gp = gpar(fontsize = 10), vjust = -1, hjust = 0.5)
#grid.text("Figure 3", vp = viewport(layout.pos.row = 3, layout.pos.col = 1:3),
#    gp = gpar(fontsize = 10), vjust = 0, hjust = 1.3)

dev.copy2eps(file = 'Figures/Figure_3.eps')
...
allGH <- sma(gh ~ SL, data = new_pento, log = "xy", method = "SMA", robust = T, slope.test = 1)
...
pento_slopes_gh <- summary(gh_lm)$coefficients[, 1]
#pento_slopes_gh <- coef(fit.gls_gh)
names(pento_slopes_gh) <- c('Pi', 'Bi', 'ZP', 'HE/DE')

all_fg_GH <- sma(gh ~ SL * j_fg, data = new_pento, log = "xy", method = "SMA", 
                 robust = T, slope.test = 1, multcomp = F)
all_fg_GH_summ <- mk_spp_summary(all_fg_GH, 4, grouping=TRUE)
all_fg_GH_graph_df <- mk_smaSPP_graph_df(all_fg_GH_summ, 4, "j_fg", iso_slope = 1)
all_fg_GH_graph_df$boot_slope <- c(pento_slopes_gh)

all_fg_GH_graph_df <- 
  all_fg_GH_graph_df %>% 
    dplyr::mutate(boot_ref_int = log10(midpoint_y / (midpoint_x ^ boot_slope))) %>% 
    dplyr::mutate(lme_slope = gh_lm_summ$estimate) %>% 
    dplyr::mutate(lme_ref_int = log10(midpoint_y / (midpoint_x ^ lme_slope)))
...
allGH_bySPP_graph_df <- mk_smaSPP_graph_df(allGH_bySPP_summ, 21, "SpeciesCode", iso_slope = 1)
...
#fig.height=7, fig.width=7,, fig.cap='**Figure 4.** Gape height ~ standard length relationships for the nine species sampled from two predatory functional groups: (a) piscivores and (b) benthic invertivores. Within each functional group, species are in order of increasing slope of the allometric relationship. Individual fish from each species are shown in open black circles, along with the fitted SMA regression for each respective species (black line). The associated sample size, regression equation and r$^2$ value are inset in each plot. The gape allometry for each species is indicated by the N (negative allometry), I (isometry), or P (positive allometry). For comparison, individual fish belonging to other species within the functional group are plotted in open grey circles; also shown is the line of isometry (slope = 1; dashed grey line) and the functional group’s fitted SMA regression (dashed black line), both plotted through the midpoint of the functional group-level relationship.'}
...
grid.layout(nrow = 5, ncol = 4, 
            widths = unit(c(0.2, 1, 0.9, 0.9), "null"),
            heights = unit(c(0.9, 1.05, 0.2, 1, 0.2), "null"))
...
grid.text(
    ...
    vjust = -1.2, gp = gpar(fontsize = 10)
    )
# benths
grid.text(
    "b)", vp = viewport(layout.pos.row = 4, layout.pos.col = 1), 
    gp = gpar(fontsize = 10), vjust = -10
    )
grid.text(
    expression( paste("Gape height (", mm, ")", sep = "") ), 
...
# Figure label
#grid.text("Figure 4", vp = viewport(layout.pos.row = 5, layout.pos.col = 1),
#    gp = gpar(fontsize = 10), hjust = -1, vjust = 1)

dev.copy2eps(file = 'Figures/Figure_4.eps')
#-------------------------------------------------------------------------------
...
#fig.height=8, fig.width=7, fig.cap='**Figure 5.** Gape height ~ standard length relationships for the (a) six zooplanktivores and (b) six energy$-$sharing herbivorous and detritivorous species, in order of increasing slope. Plot details are the same as in Figure 4.'}
# With "Figure" label
...
pushViewport(viewport(layout = master_layout))
# zoops
print(psdisp, vp = set_vp(1, 2))
print(catere, vp = set_vp(1, 3))
print(psbart, vp = set_vp(1, 4))
print(psoliv, vp = set_vp(2, 2))
print(pttile, vp = set_vp(2, 3))
print(chvand, vp = set_vp(2, 4))
# he/de
print(ceflav, vp = set_vp(4, 2))
print(acoliv, vp = set_vp(4, 3))
print(chsord, vp = set_vp(4, 4))
print(acnigr, vp = set_vp(5, 2))
print(scfren, vp = set_vp(5, 3))
print(scrubr, vp = set_vp(5, 4))
...
# Figure label
#grid.text("Figure 5", vp = viewport(layout.pos.row = 6, layout.pos.col = 1), 
#    gp = gpar(fontsize = 10), hjust = -1, vjust = 1)

dev.copy2eps(file = 'Figures/Figure_5.eps')