...
c1 <-
  ggplot(data=c_new, aes(x=log(SLMM), y=log(gh), colour=c_new$Fishing)) +
  geom_point(shape=20) +
  #geom_smooth(method=lm) +
...
gh <- ggplot(p, aes(x=SLMM, y=gh_ratio, shape=SpeciesCode, colour=SpeciesCode)) +
  geom_point(size=3) +
  scale_shape_manual(values=shapes) +
  geom_smooth(method=lm, fill=NA, size=1) 

gh
gw <- ggplot(p, aes(x=SLMM, y=gw_ratio, shape=SpeciesCode, colour=SpeciesCode)) +
  geom_point(size=3) +
  scale_shape_manual(values=shapes) +
  geom_smooth(method=lm, fill=NA, size=1)
gw
