###### Piscivores gape-body size analysis ISOMETRY ######
...
#### ADDING ratios for gh/SLMM, gw/SLMM, and ga/SLMM to p ####
...
c1 <-
  ggplot(data=p, aes(x=SLMM, y=gh)) +
  geom_point(shape=20) +
  geom_smooth(method=lm) +  
  geom_text(data=data.frame(), aes(x=100, y=50, label=lm_all_eqn(p, p$gh)), parse=TRUE) +
...
#### NON-transformed regressions (p$family) ####
...
gh_all <- mean(p$gh_ratio)
gh_all_se <- se_mean(p$_ratio)

gw_all <- mean(p$gw_ratio)
gw_all_se <- se_mean(p$gw_ratio)

ga_all <- mean(p$ga_ratio)
ga_all_se <- se_mean(p$ga_ratio)

# By family:
fam_gh_rats <- ddply(p, .(Family), summarise, "mean"=mean(gh_ratio), "se"=se_mean(gh_ratio))
fam_gw_rats <- ddply(p, .(Family), summarise, "mean"=mean(gw_ratio), "se"=se_mean(gw_ratio))
fam_ga_rats <- ddply(p, .(Family), summarise, "mean"=mean(ga_ratio), "se"=se_mean(ga_ratio))
...
# By Species:
spp_gh_rats <- ddply(p, .(SpeciesCode), summarise, "mean"=mean(gh_ratio), "se"=se_mean(gh_ratio))
spp_gw_rats <- ddply(p, .(SpeciesCode), summarise, "mean"=mean(gw_ratio), "se"=se_mean(gw_ratio))
spp_ga_rats <- ddply(p, .(SpeciesCode), summarise, "mean"=mean(ga_ratio), "se"=se_mean(ga_ratio))
