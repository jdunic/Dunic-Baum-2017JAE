...
```{r, echo=FALSE, fig.height=6, fig.width=6, fig.cap='Study site, Kiritimati (Christmas) Island, showing the 23 sampling locations around the atoll.'}
...
```{r, echo=FALSE, fig.height = 6, fig.width = 8, fig.cap = '(a) Absolute gape width (mm) and (b) relative gape width, calculated as gape width (mm) / standard length (mm) for all species ordered within each functional group by decreasing absolute gape height. A single outlier (67 mm) for *Cephalopholis argus* is not shown.'}
...
```{r, echo=FALSE, fig.height=8, fig.width=7, fig.cap='Gape width ~ standard length relationships for the nine species sampled from two predatory functional groups: (a) piscivores and (b) benthic invertivores. Plot details are the same as in Figure 4.'}
...
```{r, echo=FALSE, fig.height=8, fig.width=7, fig.cap='Gape width ~ standard length relationships for the (a) six zooplanktivore and (b) six energy$-$sharing herbivorous and detritivorous species, in order of increasing slope. Plot details are the same as in Figure 4.'}
# With "Figure" label
master_layout <- 
