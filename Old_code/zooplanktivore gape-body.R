###### Zooplanktivores gape-body size analysis ######

################        Zooplanktivores analysis             ##############
...
##### Selecting only zooplanktivores #######
...
#### calculating mouth area (and adding it to zoop dataframe): ####
zp$area <- with(zp, pi*(gh/2)*(gw/2))

#### gh, gw, ga for all zooplanktivore species combined (WITH EQNS!!!!!!) ####

df.n <- ddply(.data=zp, .(SpeciesCode), summarize, n=paste("n ==", length(SpeciesCode)))
...
c1 <-
  ...
  geom_text(data=data.frame(), aes(x=4, y=3.2, label=lm_eqn(zp, zp$gh)), parse=TRUE) +
...
c2 <-
  ...
  geom_point(shape=20) +
  geom_smooth(method=lm) +
  geom_text(data=data.frame(), aes(x=4, y=3.2, label=lm_eqn(zp, zp$gw)), parse=TRUE) +
  ggtitle("Horizontal Gape")

c3 <-
  ggplot(data=zp, aes(x=log(SLMM), y=log(area))) +
  geom_point(shape=20) +
  geom_smooth(method=lm) +
  geom_text(data=data.frame(), aes(x=4, y=5.7, label=lm_eqn(zp, zp$area)), parse=TRUE) +
  ggtitle("Gape Area")

grid.arrange(c1, c2, c3)

#### gh, gw, ga for all zooplanktivore species colour-coded ####
# (has regression lines in colour for each species)
cc1 <-
  ggplot(data=zp, aes(x=log(SLMM), y=log(gh), colour=SpeciesCode)) +
  geom_point(shape=20) +
  geom_smooth(method=lm) +
  ggtitle("Vertical Gape")

cc2 <-
  ggplot(data=zp, aes(x=log(SLMM), y=log(gw), colour=SpeciesCode)) +
  geom_point(shape=20) +
  geom_smooth(method=lm) +
  ggtitle("Horizontal Gape")

cc3 <-
  ggplot(data=zp, aes(x=log(SLMM), y=log(area), colour=SpeciesCode)) +
  geom_point(shape=20) +
  geom_smooth(method=lm) +
  ggtitle("Gape Area")
...
#### gh, gw, ga for all zooplanktivore for EACH SPP (facets) #####
# (SET OF 3 SECTIONS)
#### (1) gh plots with regression lines and equations ####
## gh lm_eqns for plotting:
spp_lms_gh <- dlply(zp, .(SpeciesCode), function(z) {
  lm(log(gh)~log(SLMM), data=z)
})

spp.n <- ddply(.data=zp, .(SpeciesCode), summarize, n=paste("n ==", length(SpeciesCode)))
...
spp1 <-
  ...
  geom_smooth(method=lm) +
  geom_text(data=gh_spp, aes(x=4, y=3.2, label=eqn), parse=T) +
...
spp_lms_gw <- dlply(zp, .(SpeciesCode), function(z) {
...
spp2 <-
  ...
  geom_smooth(method=lm) +
  geom_text(data=gw_spp, aes(x=3.9, y=3.2, label=eqn), parse=TRUE) +
...
spp_lms_ga <- dlply(zp, .(SpeciesCode), function(z) {
...
spp3 <-
  ...
  facet_wrap(~SpeciesCode, ncol=3) +
  geom_smooth(method=lm) +
  geom_text(data=ga_spp, aes(x=4.1, y=6.5, label=eqn), parse=TRUE) + 
  ggtitle("Gape Area")
facet_grid(. ~ cyl, labeller = label_both)
...
#### gh, gw, ma for all zooplanktivore for EACH FAMILY (facets) #####
# (SET OF 3 SECTIONS)
#### (1) gh plots with regression lines and equations ####
## gh lm_eqns for plotting:
fam_lms_gh <- dlply(zp, .(Family), function(z) {
  lm(log(gh)~log(SLMM), data=z)
})
## pieces for gh_summ:
...
fam1 <-
  ...
  facet_wrap(~Family) +
  geom_smooth(method=lm) +
  geom_text(data=gh_fam, aes(x=4, y=3.2, label=eqn), parse=TRUE) +
  ggtitle("VerticalGape")
facet_grid(. ~ cyl, labeller = label_both)
...
#### (2) gw plots with regression lines and equations ####
## gw lm_eqns for plotting:
fam_lms_gw <- dlply(zp, .(Family), function(z) {
  lm(log(gw)~log(SLMM), data=z)
})
## pieces for gw_summ:
...
fam2 <-
  ...
  facet_wrap(~Family) +
  geom_smooth(method=lm) +
  geom_text(data=gw_fam, aes(x=4, y=3.2, label=eqn), parse=TRUE) +
  ggtitle("Horizontal Gape")
facet_grid(. ~ cyl, labeller = label_both)
...
#### (3) ga plots with regression lines and equations ####
## ga lm_eqns for plotting:
fam_lms_ga <- dlply(zp, .(Family), function(z) {
  lm(log(area)~log(SLMM), data=z)
})
## pieces for ga_summ:
...
fam3 <-
  ...
  facet_wrap(~Family) +
  geom_smooth(method=lm) +
  geom_text(data=ga_fam, aes(x=4, y=6.2, label=eqn), parse=TRUE) +
  ggtitle("Gape Area")
facet_grid(. ~ cyl, labeller = label_both)
...
reg.all.zp <- lm(formula=log(gh)~log(SLMM), data=zp)

ggplot(data=reg.all.zp, aes())

#### gh, gw, ga lm for each spp of zooplanktivore in dataframe ####
